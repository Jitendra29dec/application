...
function update_deposit(){
        ...
        if(!empty($deposit_id)){
            if($balance_due==0){
               $query= $this->db->query('update deposit_customer set status=1,balance_due="'.$balance_due.'" where id="'.$deposit_id.'"');
               if($query){
                $response['reciptedata']=$this->order_invoice_deposit($deposit_id);
               }else{
                $response['reciptedata']='Something Wrong';
               }
            }
            else{
                $query= $this->db->query('update deposit_customer set balance_due="'.$balance_due.'" where id="'.$deposit_id.'"');
 ...
 //	echo "fmg";die;
  ...
  $appointment_row_amount = explode(',',$this->input->post('appointment_row_amount')); 
	...
	$cart_id1=$this->input->post('cart_id');
			if($cart_id1!=''){
	...
	$product_row_quantity= explode(',',$this->input->post('product_row_quantity'));
		  ...
		  /*if($product_id!=0 || $product_id!='' || $product_id!=NULL){
		    $product_id_new = explode(',',$product_id);
		  }else{
		    $product_id_new = 0;
		  }*/
/*if(!empty($cart_id)){
            foreach($cart_id as $key=>$cd){
              ...
              echo "insert into order_detail set order_id='".$order_id."', product_id='".$product_id[$key]."',tax_amount='".$product_tax_amount[$key]."',discount_amount='".$product_discount_amount[$key]."',actual_amount='".$product_row_amount[$key]."',total_paid_amount='".$product_total_amount[$key]."', quantity='".$cd->quantity."',sale_type=2";
            //  $query32 = $this->db->query("insert into order_detail set order_id='".$order_id."', product_id='".$product_id[$key]."',tax_amount='".$product_tax_amount[$key]."',discount_amount='".$product_discount_amount[$key]."',actual_amount='".$product_row_amount[$key]."',total_paid_amount='".$product_total_amount[$key]."', quantity='".$cd->quantity."',sale_type=2");
             }
          //  $this->db->query("delete from cart where cart_id IN(".$cart_id.")");
 ...
 $gift_card_value = $this->input->post('gift_card_value');
 /*$gift_card_amount = $this->input->post('gift_card_amount');
 $gift_certificate = $this->input->post('gift_certificate');*/
...
if($payment_type==1){
 ...
 if($payment_type_final=='process'){
   ...
   $query3 = $this->db->query("insert into orders set vendor_id='".$this->input->post('vendor_id')."',login_id='".$this->input->post('login_id')."', customer_id='".$customer_id."',status_id='".$status_id."', order_type='1', order_number='".$order_number."', order_amount='".$total_amount."',tip_amount='".$tip_value."',discount_amount='".$total_discount."',iou_amount='".$iou_value."',credit_card_amount='".$credit_value."',cash_amount='".$cash_value."',gift_cert_amount='".$certificate_value."',gift_cart_amount='".$gift_card_value."',rewards_money='".$reward_value."',tax_amount='".$total_tax_value."',cuppon_value='".$cuppon_value."',is_active='1',is_delete='0',is_extra_deposit='".$extra_deposit."',return_amount='".$return_amount."',tax1='".$tax1."',tax2='".$tax2."',tax3='".$tax3."',gift_certificate_number='".$certificate_number."',gift_card_number='".$gift_card_number."',created_date='".date('Y-m-d H:i')."' ");
 	//echo $this->db->last_query();exit;
 	 ...
 	 if($order_id){
			...
			*/
		      ...
		      if(!empty($cart_id)){
		        foreach($cart_id as $key=>$cd){
		        	$getCommission=$this->db->query('select commission_type,commission_amount as commission from product where product_id="'.$product_id[$key].'"')->row();
		        	if(!empty($getCommission->commission_type !='' && $getCommission->commission_type!='null')){
		        		if($getCommission->commission_type=='Percentage'){
		        			$product_commission1=$product_row_amount[$key] * $getCommission->commission;
		        			$product_commission=$product_commission1/100;
		        		}else{
    		    ...
    		    /*$this->db->query("delete from cart where cart_id ='".$cart_id[$key]."'");*/
			...
			/*if($gift_certificate_id !=''){
			    ...
			    if($gift_card_value !=''){
			      $total_cash=$this->db->query('select amount from gift_card where card_number="'.$gift_card_number.'"')->row();
			      $left_balance=$total_cash-$gift_card_value;
			      $this->db->query("update gift_card set amount='".$left_balance."' where card_number='".$_POST['gift_card_number']."'");
			      $this->db->query('insert into gift_card_history set card_number="'.$gift_card_number.'",amount="'.$gift_card_value.'"');
			    }*/
			...
			if($appointment_id_new!=0 && $appointment_id_new!=null){
				$stylist_id=$this->db->query('select service_id,stylist_id from appointment_service where appointment_id IN ('.$appointment_id_new.')')->result();
			      for($i=0;$i<count($appointment_id);$i++){
			      if($appointment_id_new!=0){
			        $stylist_ids = $stylist_id[$i]->stylist_id;
			         $service_ids = $stylist_id[$i]->service_id;
			        }else{
			        $stylist_ids = 0;
			         $service_ids =0;
			        }
			        $getCommission=$this->db->query('select commission_type,commission_amount as commission from service where service_id="'.$service_ids.'"')->row();
		        	if(!empty($getCommission->commission_type !='' && $getCommission->commission_type !='null')){
		        		if($getCommission->commission_type=='Percentage'){
		        			$service_commission1=$appointment_row_amount[$i] * $getCommission->commission;
		        			$service_commission=$service_commission1 / 100;
		        		}else{
      ...
      $this->OrderUpdate($order_id);
       if($iou_id !=''){
            ...
            if(($iou_value !='' || $iou_value !=0)){
 	 		...
 	 		$response['balance_due'] = $balance_due;
 	 		$response['reciptedata']=$this->order_invoice($order_id,$vendor_id,$role_id,$login_id);
 	...
 	*/ 	
 	...
 	if($certificate_number!='' || $gift_card_number !=''){
 			$this->db->query("update orders set gift_certificate_number='".$certificate_number."',gift_card_number='".$gift_card_number."',gift_cert_amount='".$certificate_value."',gift_cart_amount='".$gift_card_value."' where order_id='".$_POST['order_id']."'");
 	}
      ...
      /*if($iou_id !=''){
            ...
            if(($iou_value !='' || $iou_value !=0)){
 	 		...
 	 		$response['reciptedata']=$this->order_invoice($_POST['order_id'],$vendor_id,$role_id,$login_id);
 	 		 $response['balance_due'] = $balance_due;
 	 	}else{
 	 		$this->setServiceCharge($_POST['order_id'],$payment_id,$service_charge);
 	 		$payment_tra=$this->db->query('select transaction_id from payment where payment_id="'.$payment_id.'"')->row();
...
public function order_invoice_deposit($deposit_id){
  $data['customerInfo'] = $this->db->query('select A.event_name,A.deposit_amount,A.event_note,concat(B.firstname," ",B.lastname) as 	customer_name,B.email,B.mobile_phone,B.home_phone from deposit_customer as A inner join customer as B on A.customer_id=B.customer_id where id="'.$deposit_id.'"')->row();
             return $data;
       //  echo json_encode($data);
}
public function order_invoice($order_id = '',$vendor_id='',$role_id='', $login_id=''){
	//$order_id = '',$vendor_id=''
	//$order_id = '',$vendor_id=''
	//$order_id='383';
//	$vendor_id='10';
 ...
 $data['productInfo']=$this->db->query('SELECT 
  ...
  $data['gift_card']=$this->db->query('SELECT 
		...
		//	echo $message;die;
			...
			if(!empty($data['gift_certificate'])){
				$this->SendGiftCertificateMail($data['gift_certificate'],$data['customerInfo'][0]->email);
			}
			if(!empty($data['gift_card'])){
				$this->SendGiftCardMailNew($data['gift_card'],$data['customerInfo'][0]->email);
			}
...
function SendGiftCertificateMail($certificateData,$email){
//	echo "<pre>";print_r($certificateData);
	foreach($certificateData as $valCert){
				$data['business_info'] = $this->getSalonDetail($valCert->vendor_id);
				$data['gift_certificate_no']=$valCert->gift_no;
				$data['gift_image']=$valCert->gift_image;
				$subject1="New certificate";
				$message1 = $this->load->view('email_template/certificate',$data,TRUE);
				$this->send_mail->sendMail($email, $subject1, $message1, $fileName=false, $filePath=false, $cc=false);
				//mail("vardhanharsh824@gmail.com",$subject,$message1);
			}
}
function SendGiftCardMailNew($gifCardData,$email){
	//echo "<pre>";print_r($gifCardData);exit;
	foreach($gifCardData as $valCard){
				$data['business_info'] = $this->getSalonDetail($valCard->vendor_id);
				$data['gift_certificate_no']=$valCard->gift_card_no;
				$data['gift_image']=$valCard->gift_image;
				$subject2="New gift card";
				$message2 = $this->load->view('email_template/gift_card',$data,TRUE);
				$this->send_mail->sendMail($valCard->buyer_email, $subject2, $message2, $fileName=false, $filePath=false, $cc=false);
				if($valCard->recipient_email !='' && $valCard->is_myself!=1){
						$this->send_mail->sendMail($valCard->recipient_email, $subject2, $message2, $fileName=false, $filePath=false, $cc=false);
				}
				//mail("vardhanharsh824@gmail.com",$subject2,$message2);
			}
}
public function order_invoice_print(){
	$order_id =$vendor_id=$this->input->post('order_id');
	$vendor_id=$this->input->post('vendor_id');
  $this->db->query('update orders set status_id=2 where order_id="'.$order_id.'"');
   $qqrr = $this->db->query("select count(order_id) as num from orders where created_date LIKE '%".date('Y-m-d')."%' and vendor_id='".$vendor_id."' ");
	$current_date_count = $qqrr->row()->num;
...
$order_id=$this->input->post('order_id');
 ...
 $data['payment_data']=$this->db->query('select p.payment_id,p.transaction_id,IF(p.payment_type="1","Cash",IF(p.payment_type="2","Card",IF(p.payment_type="3","Credit Card",IF(p.payment_type="4","Net Banking",IF(p.payment_type="5","EBS Payments","N.A"))))) payment_type,
  ...
  //echo "<pre>";print_r($data['order_data']);exit;
  $data['customerInfo'] = $this->db->query('select concat(firstname," ",lastname) as 	customer_name,email,mobile_phone,home_phone,(select DATE_FORMAT(o.created_date,"%m/%d/%Y") from orders o where o.customer_id=customer_id order by o.created_date desc limit 1) as last_visit from customer where customer_id IN ('. $data['order_data']->customer_id.')')->result();
  /*if (!empty($data['editData']->payment_id)) {
           // $data['paymentInfo'] = $this->orders->get_payment_details($data['editData']->payment_id);
             $data['paymentInfo']=$this->db->query("SELECT payment_id,transaction_id,IF(payment_type='1','Cash',IF(payment_type='2','Debit Card',IF(payment_type='3','Credit Card',IF(payment_type='4','Net Banking',IF(payment_type='5','EBS Payments','N.A'))))) payment_type,
                         IF(status_id='1','Pending',IF(status_id='2','Success',IF(status_id='3','Reject','N.A'))) payment_status,amount FROM payment WHERE payment_id = '".$data['editData']->payment_id."'")->row();
...
if(!empty($order_id)){
if($refund_status==1){
	$getProductId=$this->db->query('select product_id,quantity from order_detail where order_id="'.$order_id.'" and sale_type=2')->result();
  ...
  $this->db->query('update gift_certificate set is_active=0 where gift_certificate_no IN ('.$gift_certificate_no.')');
    $this->db->query('update  gift_card set is_active=0 where card_number IN ('.$gift_card.')');
  ...
  foreach($getProductId as $proVal){
  	$this->db->query('update product set quantity="'.$$proVal->quantity.'" where product_id="'.$proVal->product_id.'"');
  }
  $response['status']=1;
...
public function OrderUpdate($order_id){
	$getOrderData=$this->db->query('select gift_certificate_number,	gift_card_number from  orders where order_id="'.$order_id.'"')->row();
	if(!empty($getOrderData->gift_certificate_number)){
		$this->db->query('update gift_certificate set is_active=2 where gift_certificate_no="'.$getOrderData->gift_certificate_number.'"');
	}
	if(!empty($getOrderData->gift_card_number)){
		$this->db->query('update gift_card set is_active=2 where card_number="'.$getOrderData->gift_card_number.'"');
	}
    ...
    if(!empty($gift_certificate_id)){
        for($i=0;$i<count($gift_certificate_id);$i++){
                     $this->db->query("update gift_certificate set is_active=2 where gift_id='".$gift_certificate_id[$i]->product_id."'");
            }
    }
    if(!empty($gift_cart_id)){
        ...
        for($i=0;$i<count($gift_cart_id);$i++){
        	///$this->sendGiftCardMail($gift_cart_id[$i]->product_id);
                     $this->db->query("update gift_card set is_active=2 where card_id='".$gift_cart_id[$i]->product_id."'");
            }
    }
}
   ...
   $fwrite = fwrite( $ifp, base64_decode( $data[ 0 ] ) );
	...
	public function payment_pending(){
		...
		if(!empty($token_no) && !empty($status_id)){
			...
			if(!empty($query)){
		    $response['status'] =1;
		    $response['message']='Remaining balance pending';
		  	}else{
		    $response['status'] =0;
		    $response['message'] = 'Something went wrong'; 
		 	 }
