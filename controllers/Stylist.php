...
class Stylist extends CI_Controller {
   ...
   $fwrite = fwrite( $ifp, base64_decode( $data[ 0 ] ) );
	...
	public function add(){
		...
		$query=$this->db->query("insert into login set email='".trim($email)."',password='".md5($generateTokenhash)."', is_active='".$status."', inactive_date='".$inactive_date."', role_id='3', confirmkey='".$generateTokenhash."', is_delete='0', created_date='".date("Y-m-d H:i:s")."', vendor_id='".$vendor_id."' ");
		...
		if($stylist_id){
			...
			$getData=$this->db->query('select "" as schedule_id, "" as phone, A.days as dayname,A.switch,A.start_time as new_time,(case when A.switch="0" then "00:00" else time_format(A.start_time,"%H:%i") end) as from_time,(case when A.switch="0" then "00:00" else time_format(A.end_time,"%H:%i") end) as to_time from business_hour as A  where  A.vendor_id="'.$vendor_id.'"')->result();
			$business_hour=$getData;

			foreach($business_hour as $hour){

				$schdule_id=$this->db->query("insert into schedule set stylist_id='".$stylist_id."',days='".$hour->dayname."',switch='".$hour->switch."',from_time='". $hour->from_time."',to_time='".$hour->to_time."',created_date='".date('Y-m-d h:i:s')."',vendor_id='".$vendor_id."'");
			...
			*/
			...
			$response['message'] = 'Employee added successfully';
                ...
                $this->email->message("Dear $firstname <br/>You are registered as stylist on BookNPay. Your login detail are as follow:<br/> Email: $email<br/> Password:$password <br/><br/>Thank You<br/>Team BookNPay");
                $this->email->send();
		}
		}
		}else{
	...
	public function edit(){
		...
		if(!empty($stylist_id)){
			...
			if($stored_name!=$firstname){
				
				$checkName = $this->checkNameExist($vendor_id,$firstname);
				if($checkName==1){
		...
		//$getEmail=$this->db->query('select email from login where login_id="'.$getLoginId->login_id.'" and vendor_id="'.$vendor_id.'"')->row();
		///$getEmail=$this->db->query('select email from login where email="'.$email.'" and vendor_id="'.$vendor_id.'"')->num_rows();
		//if()
		/*if($getEmail==$email){*/


		/* if($getLoginId !=''){
			$this->db->query('update login set username="'.$username.'",password="'.md5($password).'" where login_id="'.$getLoginId->login_id.'"');
		} */
		...
		if($commission_amount==''){
			...
			$this->db->query('update login set email="'.$email.'", is_active="'.$status.'", inactive_date="'.$inactive_date.'" where login_id="'.$getLoginId->login_id.'"');
		...
		/*}else{
			$getEmail1=$this->db->query('select email from login where email="'.$email.'" and vendor_id="'.$vendor_id.'"')->row();
			if(count($getEmail1) >0){
				$response['status'] = 1;
				$response['message'] = 'Email Already exsist';	
			}else{
					$this->db->query('update login set email="'.$email.'" where login_id="'.$getLoginId->login_id.'"');
		$this->db->query("update stylist set firstname='".$firstname."', lastname='".$lastname."', phone='".$phone."',email='".$email."', alternate_phone='".$alternate_phone."', country_id='231', state_id='".$state_id."', city='".$city."', address='".addslashes($address)."', type='".$employee_type."', title_id='".$title_id."',photo='".$photo_name."',hourly_rate='".$hourly_rate."' where stylist_id='".$stylist_id."' ");
				$response['status'] = 1;
				$response['message'] = 'Stylist updated successfully';
			}
		}*/
		}else{
	...
	public function getStylistById(){
		...
		if(!empty($vendor_id) && !empty($stylist_id)){
			...
			$qry = $this->db->query("select l.login_id,l.role_id,s.title_id,s.stylist_id,s.type,l.username,l.email,l.pin, s.firstname, s.lastname, s.nickname, s.phone, s.alternate_phone, c.id as country_id, st.id as state_id, s.city, s.postal_code, s.address, s.type as stylist_type, CONCAT('$path','/',s.photo) as photo,s.hourly_rate,DATE_FORMAT(s.start_date,'%m/%d/%Y') AS start_date,DATE_FORMAT(s.end_date,'%m/%d/%Y') AS end_date,s.note,s.postal_code as zipcode, s.commission_type, s.commission, IF(l.is_active=1,'Active','Inactive') as status, DATE_FORMAT(l.inactive_date,'%m/%d/%Y') AS inactive_date, s.pay_rate, s.pay_type_id from stylist s INNER JOIN login l ON l.login_id=s.login_id  LEFT JOIN countries c ON c.id=s.country_id LEFT JOIN states st ON st.id=s.state_id where l.is_delete='0' and l.vendor_id='".$vendor_id."' and s.stylist_id='".$stylist_id."' ");
			$stylistData = $qry->row();
	...
	public function checkIfStylistMobileExist(){
		...
		if($role_id==2){
			...
			if($query->num_rows()>0){
				...
				$response['status'] = 1;
				$response['message'] = 'Employee mobile already exists';
...
*/
	...
	public function getStylistInfoById($stylist_id){
		...
		$query = $this->db->query("select concat(s.firstname,' ',s.lastname) as stylist_name,s.phone,s.login_id,l.email  from stylist as s inner join login as l on l.login_id=s.login_id where s.stylist_id='".$stylist_id."' ");
		$result = $query->row();
		return $result;
	...
	public function activeDeactiveStylist(){
		...
		if(!empty($stylist_id) && !empty($vendor_id)){
			...
			//for($i=0;$i<count($stylist_id);$i++){
				$stylistInfo = $this->getStylistInfoById($stylist_id);
...
function save_availablity(){
	...
	function getScheduleData(){
		...
		if(!empty($vendor_id)){
			$getStylist=$this->db->query("select st.stylist_id as id, concat(st.firstname,' ',st.lastname) as stylist_name from stylist as st inner join login as l on l.login_id=st.login_id  inner join role as r on r.role_id=st.title_id where l.vendor_id='".$vendor_id."' and l.is_active=1 order by stylist_name asc ")->result();
	...
	public function addScheduleOutside(){
		$vendor_id=$_POST['vendor_id'];
		$stylist=$this->input->post('stylist_id');
        ...
        $color='#337ab7';
        $title_new='Schedule';
        foreach($schedule_date as $key=>$val){
...
public function AddStylistSchedule(){
	...
	if(!empty($vendor_id)){
     ...
     $data5=$this->db->query('select start_date from stylist_schedule where stylist_id="'.$stylist.'"')->result();
    ...
    $daysDate = array_diff($getDate, $data4);

    //echo "<pre>";print_r($data4);
    //echo "<pre>";print_r($getDate);
     //echo "<pre>";print_r($daysData);
   // echo "<pre>";print_r($daysDate);exit;
    $getDays=implode(",", $daysData);
    $getDateNew=implode(",",$daysDate);

   				
         
        if(!empty($daysData)){
        	 $response['status'] = 1; 
        	 $response['message'] = 'Are you sure you want to schedule employee outside availability?';
        	 $response['getDays']=$getDays;
        	 $response['getDate']=$getDateNew;
        	  $response['schedule_time']=$schedule_time;
        	   $response['schedule_end_time']=$schedule_end_time;
        	    ...
        	    /*email*/
        	    $to=$getStylistData->email;
        	   // echo $to;
        	    $data['employee_name']=$getStylistData->stylist_name;
        	    $data['business_name']=$data['business_info']->business_name;
        	   	 $data['template_type']='schedule_add';
			     $data['vendor_id']=$vendor_id;
			     $newAppointemnt=$this->db->query('select email_subject,email_content from email_settings where slug="schedule_add" and is_active=1 and vendor_id="'.$vendor_id.'"')->row();
			     $subject=$newAppointemnt->email_subject;
			     $message = $this->load->view('email_template/schedule',$data,TRUE);
			    // echo $message;die;
				...
				$getDataNew=getImageTemplate($vendor_id,'schedule_add');
				$getsmsData=str_replace($search1, $replace1, $getDataNew->sms_content);
				test($getsmsData,$getStylistData->phone);
    ...
    $row_id=	$_POST['row_id'];
    if(!empty($row_id)){
    	$getRowData=$this->db->query('select stylist_id,vendor_id from stylist_schedule where id="'.$row_id.'"')->row();
...
if($query){
			$getStylistData=$this->getStylistInfoById($getRowData->stylist_id);
        	    $data['business_info'] = $this->getSalonDetail($getRowData->vendor_id);
        	    $search1  = array('{Employee First Name}','{Business Name}');
				$employee_name=$getStylistData->stylist_name;
				$replace1 = array($employee_name,$data['business_info']->business_name);
				$getDataNew=getImageTemplate($getRowData->vendor_id,'schedule_update');
				$getsmsData=str_replace($search1, $replace1, $getDataNew->sms_content);
				//echo $getsmsData;die;
				test($getsmsData,$getStylistData->phone);
  			$response['status'] = 1; 
			$response['message'] = 'Schedule updated successfully'; 
				/*email*/
				//echo $getStylistData->email;die;
				$to=$getStylistData->email;
        	    $data['employee_name']=$getStylistData->stylist_name;
        	    $data['business_name']=$data['business_info']->business_name;
        	   	 $data['template_type']='schedule_update';
			     $data['vendor_id']=$getRowData->vendor_id;
			     $newAppointemnt=$this->db->query('select email_subject,email_content from email_settings where slug="schedule_update" and is_active=1 and vendor_id="'.$getRowData->vendor_id.'"')->row();
			     $subject=$newAppointemnt->email_subject;
			     $message = $this->load->view('email_template/schedule',$data,TRUE);
			      $this->load->library('Send_mail');
				  $this->send_mail->sendMail($to, $subject, $message, $fileName=false, $filePath=false, $cc=false);
...
public function checkIfPinAlreadyExist($login_pin){
	
	$query = $this->db->query("select login_id from login where pin='".$login_pin."' ");
	$num = $query->num_rows();
	return $num;
...
public function save_pin(){
	...
	if(!empty($login_id) && !empty($login_pin)){
		...
		$check_pin = $this->checkIfPinAlreadyExist($login_pin);
...
public function checkNameByStylistId($stylist_id){
		...
		$query = $this->db->query("select stylist_id,firstname from stylist where stylist_id='".$stylist_id."'  ");
		$res = $query->row();
	...
	function checkIfNameAlreadyExist(){
		...
		if(!empty($vendor_id)){
			...
			//echo "select count(s.stylist_id) as num from stylist s INNER JOIN login l ON l.login_id=s.login_id where CONCAT(s.firstname,' ',s.lastname)='".$name."' and l.vendor_id='".$vendor_id."' ";die;

			
			$query=$this->db->query("select count(s.stylist_id) as num from stylist s INNER JOIN login l ON l.login_id=s.login_id where CONCAT(s.firstname,' ',s.lastname)='".$name."' and l.vendor_id='".$vendor_id."' ");
			$num = $query->row()->num;
